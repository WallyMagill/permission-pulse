...
cleanup() {
    ...
    if [[ $complete -ne 1 ]]; then
        [[ $manifest_installed -eq 0 ]] || /bin/rm -f "$final_manifest"
        [[ $checksum_installed -eq 0 ]] || /bin/rm -f "$final_checksum"
        [[ $zip_installed -eq 0 ]] || /bin/rm -f "$final_zip"
    fi
...
"$repo_root/scripts/verify-release.sh" \
    "$app" "$expected_version" "$expected_build"
/usr/bin/ditto -c -k --keepParent "$app" "$temporary_zip"
"$repo_root/scripts/verify-release.sh" \
    "$temporary_zip" "$expected_version" "$expected_build"
...
/bin/mv "$temporary_zip" "$final_zip"
zip_installed=1
/bin/mv "$staging/final-checksum.txt" "$final_checksum"
checksum_installed=1
/bin/mv "$temporary_manifest" "$final_manifest"
